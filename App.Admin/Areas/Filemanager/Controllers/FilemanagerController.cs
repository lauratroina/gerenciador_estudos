...
namespace App.Admin.Areas.FilemanagerArea.Controllers
{
    /// <summary>
    ...
    public class FilemanagerController : MasterController
    {
        /// <summary>
        /// Root directory for all file uploads [string]
        /// Set in web.config. E.g. <add key="Filemanager_RootPath" value="/uploads/"/>
        /// </summary>
        private string RootPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "../"); // Root directory for all file uploads [string]

        /// <summary>
        /// Directory for icons. [string]
        /// Set in web.config E.g. <add key="Filemanager_IconDirectory" value="/Scripts/"/>
        /// </summary>
        private string IconDirectory = VirtualPathUtility.ToAbsolute("~/Filemanager/images/fileicons/"); // Icon directory for filemanager. [string]

        /// <summary>
        /// White list of allowed file extensions
        /// </summary>
        private List<string> allowedExtensions = new List<string> { ".ai", ".asx", ".avi", ".bmp", ".csv", ".dat", ".doc", ".docx", ".epub", ".fla", ".flv", ".gif", ".html", ".ico", ".jpeg", ".jpg", ".m4a", ".mobi", ".mov", ".mp3", ".mp4", ".ogv", ".webm", ".mpa", ".mpg", ".mpp", ".pdf", ".png", ".pps", ".ppsx", ".ppt", ".pptx", ".ps", ".psd", ".qt", ".ra", ".ram", ".rar", ".rm", ".rtf", ".svg", ".swf", ".tif", ".txt", ".vcf", ".vsd", ".wav", ".wks", ".wma", ".wmv", ".wps", ".xls", ".xlsx", ".xml", ".zip", ".rar", ".js", ".css", ".dll", ".config", ".json", ".aspx", ".cshtml", ".xss", ".xsc", ".rdlc", ".sql", ".asp" }; // Only allow these extensions to be uploaded
        ...
        public ActionResult Index(string mode, string path = null)
        {
            ...
            try
            {
                switch (mode)
                {
                    ...
                    case "download":
                        if (System.IO.File.Exists(RootPath + path) && IsInRootPath(path))
                        {
        ...
        {
            return path != null && Path.GetFullPath(RootPath + path).StartsWith(Path.GetFullPath(RootPath));
        }
        ...
        private string AddFolder(string path, string newFolder)
        {
            ...
            Directory.CreateDirectory(Path.Combine(RootPath + path, newFolder));
        ...
        private string Delete(string path)
        {
            ...
            if (!System.IO.File.Exists(RootPath + path) && !Directory.Exists(RootPath + path))
            {
                return Error("File not found");
            }

            FileAttributes attr = System.IO.File.GetAttributes(RootPath + path);
            ...
            {
                System.IO.File.Delete(RootPath + path);
            }
        ...
        private string GetFolderInfo(string path)
        {
            ...
            DirectoryInfo RootDirInfo = new DirectoryInfo(RootPath + path);
            StringBuilder sb = new StringBuilder();
        ...
        private string GetInfo(string path)
        {
            ...
            {
                FileInfo fileInfo = new FileInfo(RootPath + path);
                ...
                if (IsImage(fileInfo))
                {
                    using (System.Drawing.Image img = System.Drawing.Image.FromFile(RootPath + path))
                    {
                        sb.AppendLine("\"Height\": " + img.Height.ToString() + ",");
                        sb.AppendLine("\"Width\": " + img.Width.ToString() + ",");
                    }
        ...
        private string Rename(string path, string newName)
        {
            ...
            if ((attr & FileAttributes.Directory) == FileAttributes.Directory)
            {
                DirectoryInfo oldDir = new DirectoryInfo(RootPath + path);
                Directory.Move(RootPath + path, Path.Combine(oldDir.Parent.FullName, newName));
            ...
            {
                FileInfo oldFile = new FileInfo(RootPath + path);
                //Don't allow extension to be changed
        ...
        private string Replace(string path)
        {
            ...
            {
                var fi = new FileInfo(RootPath + path);
